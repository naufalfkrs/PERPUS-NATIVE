        ...
        <?php
            ...
            while($dt_histori=mysqli_fetch_array($qry_histori)){
                ...
                if(mysqli_num_rows($qry_cek_kembali)>0){
                    ...
                    $button_kembali="";
                    $hapus="<td><a href='hapushistori.php?id_peminjaman_buku=$dt_histori[id_peminjaman_buku]' onclick='return confirm(Apakah anda yakin menghapus Histori ini?)' class='btn btn-danger'>X</a></td>";
                } 
                else {
                    $status_kembali="<label class='alert alert-danger'>Belum kembali</label>";
                    $button_kembali="<a href='kembali.php?id=".$dt_histori['id_peminjaman_buku']."' class='btn btn-warning' onclick='return confirm(\"Terima Kasih\")'>Kembalikan</a>";
                }
        ?>
        <tr>
            <td><?=$no?></td>
